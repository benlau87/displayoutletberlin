...
class ZNB_Button extends ZnElements
{
	...
	function options()
	{
		...
		$options = array(
			...
			'help' => znpb_get_helptab( array(
				'video'   => 'https://my.hogash.com/video_category/kallyas-wordpress-theme/#ZZa-J_ls8WY',
	...
	function css(){
		...
		if( $button_color = $this->opt('bg_custom_color','') ){
			$css .= '.'.$uid.'.btn{background-color:'.$button_color.'}';
			$css .= '.'.$uid.'.btn:hover,.'.$uid.'.btn:focus{background-color:'.$this->opt('bg_custom_color_hover', adjustBrightness($button_color, 20) ).'}';
		}
