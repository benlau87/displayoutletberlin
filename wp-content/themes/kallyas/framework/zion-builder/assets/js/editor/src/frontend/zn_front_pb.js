 ...
 */
		...
		check_element_content : function(){
			...
			$( '.zn_pb_wrapper .zn_pb_el_container' ).each( function(){
		...
		showHideElement: function(){
			...
			$(document).on('click', '.zn_pb_hide_element_button', function(e){
				e.preventDefault();

				var $elementContainer = $(this).closest('.zn_pb_el_container'),
					isHidden		 = $elementContainer.hasClass('znklpb-element-hidden');

				if( isHidden ){
					$elementContainer.removeClass('znklpb-element-hidden');
				}
				else{
					$elementContainer.attr('data-hidden-text', $.ZnPbFactory.l10n.hidden_text);
					$elementContainer.addClass('znklpb-element-hidden');
				}

				// Change element visibility state
		...
		setElementVisibility: function( $elementContainer, isHidden ){
			// get element options
			var elementUid 		= $elementContainer.data('uid'),
				elementData = this.getElementData(elementUid);
			if( ! $.isEmptyObject( elementData ) ){
				$.ZnPbFactory.current_layout[elementUid].data.isHidden = isHidden;
			}
		},
		...
		getElementData : function( elementUid, prop ){
			...
			if ( elementUid && $.ZnPbFactory.current_layout[elementUid] ) {
				if( prop ){
					elementData = $.ZnPbFactory.current_layout[elementUid]['data'][prop];
				}
				else{
					elementData = $.ZnPbFactory.current_layout[elementUid];
				}
			}
 ...
 */
		sortable_arguments : function( scope ) {
			...
			var fw = this,
				...
				tolerance = ( scope == 'content_element' ) ? 'pointer' : 'intersect';
			...
			return {
				...
				change: function( event, ui ){
					if ( scope !== 'content_element' ) {
						var helper_height = $(ui.helper).prev().height();
		...
		import_template : function() {
			...
			// Add behavior for the template saving
			$('#znpb_upload_input').change(function(){

				var $this = $(this),
					uploadbtn = $('.js-znpb_upload_template'),
					...
					success: function(response){
						...
						if ( response.message ) {
							...
							if( response.isSingle === true ){
								var draggableEl = saved_el.children('.zn_pb_element');
								draggableEl.draggable(fw.get_draggable_options( draggableEl.data('level') ));
		...
		export_indv_template : function(){
			...
			fw.body.on('click', '.zn_pb_export_indv_template' , function(e){
				...
				var el = $(this),
					...
					element = container.children('.zn_pb_element'),
		...
		get_values : function(el){
			...
			if ( element_uid && $.ZnPbFactory.current_layout[element_uid] ) {
				values = $.ZnPbFactory.current_layout[element_uid].data.options;
			}
					...
					if ( el.data('has_multiple') ) {
						...
						if( fw.stop_order && fw.stop_order != 'undefined' && ! $.isEmptyObject(fw.stop_order) ) {
							...
							$.each( cached_stop_order, function( k, v ){
								...
								//  Refresh UID's if this element was cloned
		...
		add_to_factory : function( data ){
			$.each(data,function(){
				$.ZnPbFactory.current_layout[this.data.uid] = this;
			});
			...
			var elements = (scope) ? scope.find('.zn-options-tab-header > a') : $('.zn-options-tab-header > a');
				...
				elements.on( 'click', function(e){
					...
					var tab = $(this).data("zntab"),
						modal = $(this).closest('.zn-modal-form');
			...
			// Adds/Removes a css class
				...
				if ( input.attr('type') == 'checkbox' ) {
					...
					if ( input.attr('type') == 'radio' ) {
						// Get proper values
						values = $.map( $('input' , that ), function(option) {
							return option.value;
						});
						...
						values = $.map( $('select option' , that ), function(option) {
							return option.value;
						});
					}
					...
					$.each( tasks, function( idx, task ){
						...
						if ( can_set ){
							if( condition_type === 'set' ){
								$( css_class ).css( property, property_value );
							}

							if( condition_type === 'remove' ){
								$( css_class ).css( property, '' );
							}
						}
			...
			element.click(function(e){
				...
				jQuery.post( ZnAjax.ajaxurl, data, function( response ) {
					...
					if ( response.message ) {
						new $.ZnModalMessage( response.message );
						var saved_el = $(response.content),
							draggableEl = saved_el.children('.zn_pb_element');
						draggableEl.draggable(fw.get_draggable_options( draggableEl.data('level') ));
		...
		show_element_options :	function() {
			...
			fw.scope.on( 'click', '.znpb-element-options-trigger', function(e){
				...
				if ( typeof options.data !== 'undefined' && typeof options.data.content !== 'undefined' && ! $.isEmptyObject(options.data.content) ) {
				...
				params.modal_on_ajax_load = function(e){
					...
					if( main_element.data('has_multiple') ){
						...
						// Change the order when an item is removed
						// Allows us to copy the content of multiple content elements
						sort_option.find('.zn_clone_button').on('znpb:element:cloned', function( event, oldContent, newContent ){
							var clonedElementIndex = newContent.index(),
								oldContentElementIndex = oldContent.index();
		...
		update_el : function(scope) {
			...
			if( typeof $.ZnPbFactory.current_layout[element_uid] != 'undefined' ) {
				$.ZnPbFactory.current_layout[element_uid].data.options = fw.get_form_values(form);
			}
			else {
				$.ZnPbFactory.current_layout[element_uid].data.options = fw.get_form_values(form);
			}
