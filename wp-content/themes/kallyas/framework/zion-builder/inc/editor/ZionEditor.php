...
class ZionEditor
{
	...
	public function init()
	{
		...
		// Add templates
		add_action( 'wp_footer', array( $this, 'addTemplates' ) );
		...
		add_action( 'zn_pb_content', array( $this, 'setupDummyContent' ) );
	 ...
	 *
	 */
	public function setupDummyContent()
	{
		$args = array(
	...
	public function addTemplates()
	{
		...
		<script type="text/html" id="tmpl-znfb-pbelement-content">
			<div class="zn_pb_element ui-draggable ui-draggable-handle"
				 data-keywords="{{{ data.name }}}, {{{ data.keywords }}}" data-object="{{data.class}}"
				 data-level="{{data.level}}" data-widget="{{data.widget_id}}">
	...
	public function loadScripts()
	{
		...
		wp_enqueue_style( 'alloy-editor-min', ZNB()->plugin_url . '/assets/js/editor/alloy-editor/assets/alloy-editor-ocean.css', array(), ZNB()->version );

		// PAGE BUILDER CUSTOM SCRIPTS
		wp_enqueue_script( 'zn_front_pb', ZNB()->plugin_url . '/assets/js/editor/frontend_pagebuilder.bundle.js', array(
			'jquery',
			'zn_html_script',
			'backbone',
			'underscore',
		...
		wp_localize_script( 'zn_front_pb', 'ZnKlPb', array(
			'ALLOYEDITOR_BASEPATH' => ZNB()->plugin_url . '/assets/js/editor/alloy-editor/',
			'CKEDITOR_BASEPATH'    => ZNB()->plugin_url . '/assets/js/editor/alloy-editor/',
		) );
