...
function znpb_render_module() {
	...
	if( ! empty( $_POST['template_name'] ) ) {
		$post_id = $_templates->getPostID( 'zn_pb_el_templates' );
		$template_name = esc_attr( stripslashes(str_replace(array(' '), '_', strip_tags($_POST['template_name']))) );
		$template_data = $_templates->getPageBuilderTemplates( 'zn_pb_el_templates', $template_name , '=' );
...
function znpb_do_save_element( $template_data = false ){
	...
	if ( $result ){
		...
		$return['content'] = $zTemplates->getSavedElementMarkup( $template_name, $content );
	}
	else{
...
function zn_save_template(){
	...
	if ( $result ){
		...
		$return['content'] = $zTemplates->renderTemplate( $template_name );
	}
	else{
...
function zn_delete_saved_element(){
	...
	$post_id = ZNB()->templates->getPostID( 'zn_pb_el_templates' );
...
function zn_load_template()
{
	...
	$post_id = $znb->templates->getPostID();
	$template_data = $znb->templates->getPageBuilderTemplates( 'zn_pb_templates', $_POST['template_name'] , '=' );

	if ( !empty( $template_data[0] ) ) {

		$template_data = maybe_unserialize($template_data[0]);
		$pb_template = znb_generate_uid( $template_data['template'] );
		...
		// Setup elements
		$znb->frontend->setupElements( $pb_template );

		// Start output fetching
		ob_start();
			do_shortcode( $znb->frontend->renderContent( $pb_template ) );
		$html = ob_get_contents();

		$response['template'] = do_shortcode( $html );
		$response['current_layout'] = $znb->frontend->getLayoutModules();
		$response['custom_css'] = $custom_css;
		$response['custom_js'] = $custom_js;
		ob_end_clean();
	}
	else {
		$response['message'] = __('There was a problem loading the template', 'zn_framework');
	}
