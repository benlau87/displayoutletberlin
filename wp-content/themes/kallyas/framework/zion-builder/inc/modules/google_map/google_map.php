...
class ZNB_GoogleMap extends ZionElement
{
	...
	function element(){
		...
		if ( !$this->validation('locations') ) {
			echo '<div class="zn-pb-notification">Please configure the element options and add <u>at least one location</u>.</div>';
			return;
		}
			...
			<?php if ( $this->opt('map_directions') === 'yes') {?>
				<div class="zn-visit-container zn-visit--pos-<?php echo $this->opt('map_directions_pos','top-left'); ?>">
					...
					<span class="zn-visit-removeRoute">
						<?php echo znb_get_svg(array('icon'=>'close-thin')) ?>
					</span>
				</div>
			<?php } ?>
