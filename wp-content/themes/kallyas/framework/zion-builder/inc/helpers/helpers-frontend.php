...
function znb_background_source( $args = array() )
{
	...
	if ( $args['source_overlay'] != 0 ) {
		...
		if(strpos($overlay_color, 'rgba') === false ){
			if( $overlay_opac = $args['source_overlay_opacity'] ){
		...
		if ( $args['source_overlay'] == 2 || $args['source_overlay'] == 3 ) {
			...
			if(strpos($gr_overlay_color, 'rgba') === false ){
				if( $overlay_gr_opac = $args['source_overlay_color_gradient_opac'] ){
	...
	if($args['mobile_play'] == 'yes'){
		...
		if(!empty($mp_video)){
			echo '<a href="'.$mp_video.'" class="mfp-iframe zn-bgSource-videoModal visible-xs visible-sm" data-text="'. __('PLAY VIDEO', 'zn_framework') .'">'. znb_get_svg(array('icon'=>'play')) .'</a>';
		}
	}
...
function znb_VideoBg( $args = array() )
{
	...
	$classes[] = 'zn-videoBg-gridOverlay zn-videoBg-gridOverlay--' . $args['source_vd_overlay'];
 ...
 */
add_action('wp_footer', 'znb_animateFallback', 10);
