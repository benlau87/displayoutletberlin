...
class ZionPageBuilderFrontend
{
	...
	function loadScripts()
	{
		...
		wp_enqueue_script('zion-frontend-js', ZNB()->plugin_url . '/assets/js/editor/zn_frontend.js', array('jquery'), ZNB()->version, true);

		// 1. Load element extra scripts
		if ( !empty( $this->_registered_layout_areas ) ) {
			foreach ( $this->_registered_layout_areas as $key => $area_config ) {
				if ( is_array( $area_config[ 'layout_data' ] ) ) {
					foreach ( $area_config[ 'layout_data' ] as $key => $element ) {
						if ( !empty( $this->instantiated_modules[ $element[ 'uid' ] ] ) ) {
							$element_instance = $this->instantiated_modules[ $element[ 'uid' ] ];
							$element_instance->scripts();
						}
					}
				}
			}
