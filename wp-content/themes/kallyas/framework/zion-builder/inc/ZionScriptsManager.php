...
class ZionScriptsManager
{
	...
	function compileElementsCss( $recompile = false )
	{
		...
		foreach( ZNB()->elements_manager->getElements() as $class ) {
			...
			if ( $fs->is_file( $class->path . '/style.css') ) {
				$css .= $fs->get_contents( $class->path . '/style.css' );
			}

			// Check to see if we have an style.php file
			if( $fs->is_file( $class->path.'/style.php') ){
				ob_start();
					include( $class->path.'/style.php' );
				$css .= ob_get_clean();
			}
		}
		...
		$fs->put_contents( $assetsConfig['path'] . 'znb_compiled_css.css', $css_code);
	...
	public function createAssetCss( $post_id = null )
	{
		...
		foreach ( $current_layout as $key => $element_instance ) {
			if ( !isset( $loaded_assets[ $element_instance->data[ 'object' ] ] ) ) {
				...
				$filePath = trailingslashit( $element_instance->path ) . 'style.css';
				if ( $fs->is_file( $filePath ) && $fs->is_readable( $filePath ) ) {
					$css .= $fs->get_contents( $filePath );
				}

				// Add the style.php file if present
				$filePath = trailingslashit ($element_instance->path ) . 'style.php';
				if ( $fs->is_file( $filePath ) && $fs->is_readable( $filePath ) ) {
					ob_start();
					include( $filePath );
					$css .= ob_get_clean();
		...
		return $fs->put_contents( $filePath, $css );
	}
