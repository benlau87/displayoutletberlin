	...
	function zn_pagination( $args = array() )
	{
		...
		if( (int)$args['pages'] > 1 )
		{
			...
			if ( false !== $args['previous_text'] ) {
				if( 1 != $args['paged'] ) {
					$output .= '<li class="pagination-item pagination-item-prev pagination-prev">'. $isComment ? get_previous_comments_link( $args['previous_text'] ) : get_previous_posts_link($args['previous_text']).'</li>';
				}
				else{
			...
			if ( false !== $args['older_text'] ) {
				if ( $args['paged'] < $args['pages'] ) {
					$output .= '<li class="pagination-item pagination-item-next pagination-next">' . $isComment ? get_next_comments_link( $args['older_text'] ) : get_next_posts_link( $args['older_text'] ) . '</li>';
				}
				else {
