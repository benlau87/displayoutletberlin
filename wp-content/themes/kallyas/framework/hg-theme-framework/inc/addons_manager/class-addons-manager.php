...
class ZnAddonsManager {
	 ...
	 */
	 function __construct() {
		 ...
		 if( is_admin() ){
			 ...
			 // The Api plugins should only be registered if the theme is connected
	 ...
	 function setupApiPlugins(){
		 // Load API plugins only if the theme is connected
	...
	public function get_download_url( $slug, $pluginSource = '' )
	{
		...
		switch ( $pluginSource )
		{
			case 'internal':
			{
				$response = ZN_HogashDashboard::getPluginDownloadUrl( $slug, $this->plugins[ $slug ][ 'source' ] );
