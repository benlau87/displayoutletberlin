...
class ZN_HogashDashboard
{
	...
	public static function connectTheme( $apiKey )
	{
		...
		$response = self::request( $sendData );
	...
	public static function getAllDemos()
	{
		...
		if ( !$data[ 'success' ] )
		{
			return array( 'error' => sprintf( __( '[0003]  Response from server: %s', 'zn_framework' ), $data[ 'data' ] ) );
		}
	...
	public static function getAllPlugins()
	{
		...
		if ( !$data[ 'success' ] )
		{
			return array( 'error' => sprintf( __( '[0003]  Invalid response from server: %s', 'zn_framework' ), $data[ 'data' ] ) );
		}
	...
	public static function request( $args = array() )
	{
		...
		$response = wp_remote_post( ZNHGTFW()->getThemeServerUrl(), $args );
		return $response;
	}
	...
	{
		$apiKey = ( self::isManagedApiKey() ? self::getManagedApiKey() : self::getThemeApiKeyOption() );
	...
	public static function deleteThemeCheckOption() {
		...
		if( self::isGoDaddy() ){ wp_using_ext_object_cache( false ); }
		if( self::isManagedApiKey() ){
			return delete_site_option( self::getOptionsPrefix() . self::THEME_CHECK_OPTION );
		}
		return delete_transient( self::getOptionsPrefix() . self::THEME_CHECK_OPTION );
	}
	...
	public static function deleteThemeApiKeyOption() {
		...
		return delete_option( self::getOptionsPrefix() . self::THEME_API_KEY_OPTION );
	}

	/**
	 * Save the default settings when the API Key is managed
	 * @param string $apiKey
	 */
	public static function saveManagedSettings( $apiKey )
	{
		self::setThemeApiKeyOption( $apiKey );
		self::setThemeCheckOption( '1x' );
	}
