var project  	  = 'hg-framework',
	text_domain   = project;

// Style related.
//
var scssSRC                = './assets/src/scss'; // Path to main .scss file.
var styleSRC                = scssSRC + '/hgfw.scss'; // Path to main .scss file.
 ...
 *
*/
var frontendJsPath = './assets';
var frontendJsSource = frontendJsPath + '/src/js/frontend/*.js';
gulp.task( 'frontendJS', function() {
	return gulp.src( frontendJsSource )
		.pipe(plumber())
		.pipe( concat( 'hgfw_frontend.js' ) )
		.pipe( uglify() )
		.pipe( lineec() ) // Consistent Line Endings for non UNIX systems.
		.on('error', gutil.log)
		.pipe( gulp.dest( frontendJsPath + '/dist/js' ) );
});
  ...
  */
 gulp.task( 'default', ['styles', 'frontendJS'], function () {
...
gulp.task('build', function(cb) {
	runSequence('styles', 'frontendJS', 'translate', 'changeVersion', 'buildFiles', 'buildZip', 'cleanup', 'finish');
});
