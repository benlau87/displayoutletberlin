...
class ZnHgFw_IconManager {
	...
	public function upload_icons() {
		...
		if ( !isset( $_POST[ 'attachment' ] ) ) {
			wp_send_json( array( 'message' => esc_html( __( 'Invalid request', 'zn_framework' ) ) ) );
		}
		$attachment = $_POST[ 'attachment' ];
		if ( empty( $attachment ) || !is_array( $attachment ) || !isset( $attachment[ 'id' ] ) || !isset( $attachment[ 'title' ] ) ) {
			wp_send_json( array( 'message' => esc_html( __( 'Invalid request. Missing attachment', 'zn_framework' ) ) ) );
		}
	...
	public function remove_icons() {
		if ( !isset( $_POST[ 'font_name' ] ) ) {
			wp_send_json( array( 'message' => esc_html( __( 'Invalid request. Missing font name', 'zn_framework' ) ) ) );
		}
		...
		if ( !is_dir( $dirPath ) ) {
			wp_send_json( array( 'message' => esc_html( __( 'Invalid request. Directory not found', 'zn_framework' ) ) ) );
		}
	...
	public function do_icon_install( $path, $title ) {
		...
		if ( isset( $unzipped[ 'error' ] ) && !empty( $unzipped[ 'error' ] ) ) {
			...
			if ( isset( $font_data[ 'message' ] ) && !empty( $font_data[ 'message' ] ) ) {
				...
				$return[ 'message' ] = esc_html( sprintf( __( ' The %s font was successfully added', 'zn_framework' ), $this->font_name ) );
	...
	public function create_data() {
		...
		if ( empty( $svg_file ) ) {
			...
			$return[ 'message' ] = esc_html( __( 'The zip did not contained any svg files.', 'zn_framework' ) );
			return $return;
		}
		//#! since v4.1.4
		...
		if ( !is_file( $svgFile ) || !is_readable( $svgFile ) ) {
			zn_delete_folder( $this->paths[ 'tempdir' ] );
			$return[ 'message' ] = esc_html( __( 'Could not find or read the svg file.', 'zn_framework' ) );
			return $return;
		}
		...
		if ( !is_wp_error( $file_data ) && !empty( $file_data ) ) {
			...
			if ( !empty( $icon_list ) && !empty( $this->font_name ) ) {
				...
				if ( $icon_list_file ) {
					...
					$fs->put_contents( $icon_list_file, $strData );
				}
				else {
					zn_delete_folder( $this->paths[ 'tempdir' ] );
					$return[ 'message' ] = esc_html( __( 'There was a problem creating the icon list file', 'zn_framework' ) );
					return $return;
				}
			...
			$return[ 'message' ] = esc_html( __( 'The svg file could not be opened.', 'zn_framework' ) );
		}
		return $return;
	}
	...
	*/
	public function do_icons_archive( $zip ) {
		...
		if ( !$tempdir ) {
			$return[ 'error' ] = esc_html(__('The temp folder could not be created !', 'zn_framework'));
			return $return;
		}
		WP_Filesystem();
		...
		if ( $extracted ) {
			...
			$return[ 'error' ] = esc_html(__('The zip file could not be extracted !', 'zn_framework'));
		}
		return $return;
	}
