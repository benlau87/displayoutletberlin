	...
	class WpkPageHelper
	{
		...
		public static function zn_background_source( $args = array() )
		{
			...
			if( function_exists('znb_background_source') ){
				...
				return znb_background_source($args);
			}


			$parallax = $sourceType == 'image' && $args['enable_parallax'] == 'yes' && !ZNB()->utility->isActiveEditor();
			$bg_container_class='';

			if ( $sourceType ):

				// IMAGE
				if ( $sourceType == 'image' && isset($args['source_background_image']['image']) && ( $background_image = $args['source_background_image']['image'] ) ) {
					...
					if ( $parallax ) {
						$bg_container_class = 'kl-bg-source__parallax js-znParallax';
					}
					else {
		...
		public static function zn_klvideo( $args = array() )
		{
			...
			$output = '<div class="kl-video-container '.$args['wrapper_class'].'">';
				...
				$output .= '<div class="kl-video kl-video--valign kl-video--halign '.$args['video_class'].'" data-setup=\''.json_encode($video_options).'\' '.WpkPageHelper::zn_schema_markup('video').'></div>';
			$output .= '</div>';
